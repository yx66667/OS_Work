    ...
    for (i = 0; i < memmap->nr_map; i ++) {
        ...
        if (memmap->map[i].type == E820_ARM) {	//第二次遍历物理内存，这一次遍历，主要是调用init_memmap初始化各个页表。
            ...
            if (begin < end) {
                ...
                if (begin < end) {
                    init_memmap(pa2page(begin), (end - begin) / PGSIZE); //pa2page（begin）作用是返回传入参数pa开始的第一个物理页，其基地址base
...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    pde_t *pdep = &pgdir[PDX(la)];  //la是线性地址，32位，需要提取出该字段内容，才能获取页表内容。
    if (!(*pdep & PTE_P)) {  //如果获取不成功，那么需要根据create标记位来决定是否创建这一个二级页表（注意，一级页表中，存储的都是二级页表的起始地址）。
        struct Page *page;                              //如果create为0，那么不创建，否则创建。
        if (!create || (page = alloc_page()) == NULL) { 
            return NULL;
        }
        set_page_ref(page, 1); //set_page_ref(page,1): 设置此页被引用一次
        uintptr_t pa = page2pa(page); //page2pa(page): 得到page管理的那一页的物理地址
        memset(KADDR(pa), 0, PGSIZE); //KADDR(pa): 返回pa对应的虚拟地址（线性地址）
        //使用memset将新建的这个页表虚拟地址，全部设置为0，因为这个页所代表的虚拟地址都没有被映射。
        *pdep = pa | PTE_U | PTE_W | PTE_P; //设置控制位。这里应该设置同时设置上PTE_U、PTE_W和PTE_P，
        //分别代表用户态的软件可以读取对应地址的物理内存页内容、物理内存页内容可写、物理内存页存在。
    }
